...
const LeaveRequest = () => {
  ...
  const handleSubmitOrUpdate = (e) => {
    ...
    if (isEditing) {
      if (role !== "admin") {
        alert("Only admins can update leave requests.");
        return;
      }
  ...
  const updateStatus = (id, newStatus) => {
    if (role !== "admin") {
      alert("Only admins can approve or reject leave requests.");
      return;
    }
  ...
  const handleEdit = (id) => {
    if (role !== "admin") {
      alert("Only admins can edit leave requests.");
      return;
    }
  ...
  const handleDelete = (id) => {
    if (role !== "admin") {
      alert("Only admins can delete leave requests.");
      return;
    }
      ...
      {activeLeaves.length === 0 ? (
        ...
        <ul className="list-group">
          {activeLeaves.map((leave) => (
            ...
            >
              <span>
                {leave.type} leave - <strong>{leave.startDate}</strong> to{" "}
